...
class Registration extends Component {
  componentWillMount() {
    ...
    const user = users.find(({ tournaments }) => tournaments.find(({ _pivot_id }) => _pivot_id === registrationId));
    if (!user) {
  ...
  render() {
    ...
    const notFound = (
      <div className="container">
        <h2 className="py-4">Registration not found. You might not have permission to see this registration.</h2>
      </div>
    );
    ...
    const registrationTableRows = [
      ...
      tournament._pivot_price_owed
        ? ['Costs', <Currency quantity={tournament._pivot_price_owed || 0} currency="EUR" />] : null,
      tournament._pivot_price_paid
        ? ['Price paid', <Currency quantity={tournament._pivot_price_paid || 0} currency="EUR" />] : null,
      tournament._pivot_price_owed
        ? [
          'Debt',
          (
      ...
      tournament._pivot_partner1
        ? [`${tournament._pivot_partner2 ? 'Partner 1' : 'Partner'}`, tournament._pivot_partner1] : null,
        ...
        {(user && user.tournaments) ? (
          <div className="row">
            <div className="col-12 col-md-6 col-lg-5 col-xl-4 offset-md-1">
              ...
              <FlexTable key={`registrationTable_${tournament._pivot_id}`}
